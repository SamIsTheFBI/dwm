...
struct Client {
	...
	int isfixed, isfloating, isurgent, neverfocus, oldstate, isfullscreen, isterminal, noswallow, cantfocus;
...
drawbar(Monitor *m)
{
	...
	if ((w = mw - tw - stw - x) > bh) {
      if (m->sel) {
			drw_setscheme(drw, scheme[SchemeNorm]);
	        drw_text(drw, x, y, w - 2 * sp, th, lrpad / 2 + (m->sel->icon ? m->sel->icw + ICONSPACING : 0), m->sel->name, 0);
  ...
  if (display != NULL) {
    ...
    if (resm != NULL) {
      ...
      if (xrdb != NULL) {
        ...
        XRDB_LOAD_COLOR("dwm.sel_fg", selfgcolor);
